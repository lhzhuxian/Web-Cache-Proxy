...
bool Socket::connect() {
    ...
    printf("connect, fd=%d, addr=%s\n", _socketHandle, getAddr().c_str());
...
int Socket::read (void *data, int len) {
    ...
    do {
        ...
        if (res > 0) {
	  // printf("read, fd=%d, addr=%d", _socketHandle, res);
        }
    } while (res < 0 && errno == EINTR);
    return res;
}
...
bool Socket::setTimeOption(int option, int milliseconds) {
    ...
    if (checkSocketHandle()) {
        ...
        rc = (setsockopt(_socketHandle, SOL_SOCKET, option,(const void *)(&timeout), sizeof(timeout)) == 0);
    }
    return rc;
}
...
bool Socket::setTcpNoDelay(bool noDelay) {
    ...
    if (checkSocketHandle()) {
        rc = (setsockopt(_socketHandle, IPPROTO_TCP, TCP_NODELAY,(const void *)(&noDelayInt), sizeof(noDelayInt)) == 0);
    }
    return rc;
}
...
bool Socket::setTcpQuickAck(bool quickAck) {
  ...
  if (checkSocketHandle()) {
    rc = (setsockopt(_socketHandle, IPPROTO_TCP, TCP_QUICKACK,(const void *)(&quickAckInt), sizeof(quickAckInt)) == 0);
  }
  return rc;
}
...
uint64_t Socket::getId() {
	return ipToAddr(_address.sin_addr.s_addr,ntohs(_address.sin_port));
}
